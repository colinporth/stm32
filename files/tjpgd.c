...
//}}}
//{{{
static JRESULT mcu_output (JDEC* jd, UINT (*outfunc)(JDEC*, void*, JRECT*), UINT mcux, UINT mcuy) {
  ...
  /* Output rectangular size (it may be clipped at right/bottom end) */
  rx = (mcux + mx <= jd->width) ? mx : jd->width - mcux;
  ry = (mcuy + my <= jd->height) ? my : jd->height - mcuy;
  ...
  if (jd->scale == 3) {
    //{{{  1/8 scaling (left-top pixel in each block are the DC value of the block) */
  ...
  return outfunc (jd, jd->workbuf, &rect);
}
//}}}
