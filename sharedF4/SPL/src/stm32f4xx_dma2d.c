...
/**
  * @brief  Define the configuration of the line watermark .  DMA2D->CR = (DMA2D->CR & 0xFFFCE0FC) | DMA2D_M2M_BLEND;        // reset flags, set mode
  DMA2D->OPFCCR = DMA2D_RGB565;                                  // colour format
  DMA2D->OCOLR = colour;                                         // fixed colour
  DMA2D->OMAR = CurrentFrameBuffer + ((y*TFTWIDTH) + x)*2; // output memory address
  DMA2D->OOR = TFTWIDTH - pitch;                                 // line offset
  DMA2D->NLR = (pitch << 16) | rows;                             // xlen:ylen
